...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@opencodex/opencodex structure
  const pkgDir = join(tmpDir, 'node_modules', '@opencodex', 'opencodex')
...
describe('package consumer types', () => {
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@opencodex/opencodex/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@opencodex/opencodex/sdk'`,
        ``,
