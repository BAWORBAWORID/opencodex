...
Manage the OpenCodex project wiki stored in .openclaude/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized OpenCodex wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `OpenCodex wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  return [
    'OpenCodex wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the OpenCodex wiki.`,
    '',
    `Title: ${result.title}`,
