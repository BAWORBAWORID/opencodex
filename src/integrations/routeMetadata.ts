...
export function hasXiaomiMimoEnvOnlyProviderIntent(
  ...
  return (
    ...
    !hasNonEmptyEnvValue(processEnv.MINIMAX_API_KEY) &&
    !hasNonEmptyEnvValue(processEnv.VENICE_API_KEY) &&
    !hasNonEmptyEnvValue(processEnv.OPENGATEWAY_API_KEY) &&
    !hasConflictingOpenAIBaseUrlForRoute(processEnv, isXiaomiMimoBaseUrl) &&
    hasNoExplicitNonOpenAICompatibleProvider(processEnv)
  )
}

export function isOpengatewayBaseUrl(value: string | undefined): boolean {
  const trimmed = value?.trim()
  if (!trimmed) {
    return false
  }
...
export function getOpengatewayBaseUrlOverride(
  processEnv: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const openAIBaseUrl = processEnv.OPENAI_BASE_URL?.trim()
  if (isOpengatewayBaseUrl(openAIBaseUrl)) {
    return openAIBaseUrl
  }

  const openAIApiBase = processEnv.OPENAI_API_BASE?.trim()
  if (isOpengatewayBaseUrl(openAIApiBase)) {
    return openAIApiBase
  }
...
export function hasOpengatewayEnvOnlyProviderIntent(
  processEnv: NodeJS.ProcessEnv = process.env,
): boolean {
  return (
    hasNonEmptyEnvValue(processEnv.OPENGATEWAY_API_KEY) &&
    ...
    !hasNonEmptyEnvValue(processEnv.VENICE_API_KEY) &&
    !hasNonEmptyEnvValue(processEnv.MIMO_API_KEY) &&
    !hasConflictingOpenAIBaseUrlForRoute(processEnv, isOpengatewayBaseUrl) &&
    hasNoExplicitNonOpenAICompatibleProvider(processEnv)
  )
}
...
export function resolveEnvOnlyProviderRouteId(
  processEnv: NodeJS.ProcessEnv = process.env,
): 'xai' | 'minimax' | 'venice' | 'xiaomi-mimo' | 'opengateway' | null {
  ...
  if (hasOpengatewayEnvOnlyProviderIntent(processEnv)) {
    return 'opengateway'
  }
