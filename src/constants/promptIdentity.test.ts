...
test('CLI identity prefixes describe OpenCodex instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('OpenCodex')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('OpenCodex')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes OpenCodex instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('OpenCodex')
...
test('built-in agent prompts describe OpenCodex instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('OpenCodex')
  ...
  expect(generalPrompt).toContain('OpenCodex')
  ...
  expect(explorePrompt).toContain('OpenCodex')
  ...
  expect(guidePrompt).toContain('OpenCodex')
  expect(guidePrompt).toContain('You are the OpenCodex guide agent.')
  expect(guidePrompt).toContain('**OpenCodex** (the CLI tool)')
